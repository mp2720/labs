...
let rec json toks =
  let jnumber =
    ...
    let rec digits start toks =
      ...
      (digit
      <*> (digit_range '0' '9' <*> digits start =>> ( ^ ) <|> empty "")
      =>> ( ^ ))
        toks
    in

    let int_part =
      zero <|> (digit_range '1' '9' <*> (digits '0' <|> empty "") =>> ( ^ ))
    in
  ...
  let compound left item right =
    ...
    let rec comma_items toks =
      (comma_item <*> comma_items =>> List.cons <|> empty []) toks
    in
  ...
  (whitespace <*> value <*> whitespace => fun ((_, v), _) -> v) toks
